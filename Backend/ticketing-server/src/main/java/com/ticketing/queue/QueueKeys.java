...
public class QueueKeys {
    ...
    public static String waitingZSet(String matchId){
        return "queue:%s:waiting".formatted(matchId);
    }

    // 방별 사용자 상태 키 변수
    public static String userStateKey(String matchId, String userId){
        return "queue:%s:%s".formatted(matchId, userId);
    }

    // 방별 순서 계산을 위한 시퀀스 변수
    public static String sequence(String matchId){
        return "queue:%s:seq".formatted(matchId);
    }

    // 내 앞에 빠진 사람 수 누적을 위한 변수
    public static String roomOffset(String matchId){
        return "queue:%s:offset".formatted(matchId);
    }

    // 빠진 사람만큼 방 내 최대 인원 반영을 위한 변수
    public static String roomTotal(String matchId){
