...
export default function PersonalStats({ matchHistory }: PersonalStatsProps) {
  ...
  const averageRank =
    myData.length > 0
      ? (
          myData.reduce((sum, data) => sum + data.rank, 0) / myData.length
        ).toFixed(1)
      : "-";

  // 차트 데이터 포맷팅
  const chartData = myData.map((data) => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-purple-200 bg-purple-50 p-6">
        <h3 className="mb-4 text-lg font-bold text-neutral-900">평균 등수</h3>
