...
export default function UserStats({
  ...
  const getTotalParticipants = (match: MatchHistory): number => {
    ...
    // tags에서 봇 인원 추출
    const botTag = match.tags?.find((tag: { label: string }) => tag.label.includes("봇"));
    if (botTag) {
  ...
  const userData = matchHistory
    .filter((match) => match.users?.some((user: { id: number }) => user.id === userId))
    .map((match) => {
      const myUser = match.users?.find((user: { id: number }) => user.id === userId);
